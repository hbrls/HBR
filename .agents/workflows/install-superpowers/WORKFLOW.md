...
```bash
mkdir -p .context
git clone --single-branch --depth 1 git@github.com:obra/superpowers.git .context/superpowers
```
...
1. Remove Gemini- and Copilot-related content. We do not use Gemini or GitHub Copilot.

Delete any tool mapping files for these platforms if present, for example:

```bash
rm -f .agents/skills/using-superpowers/references/gemini-tools.md
rm -f .agents/skills/using-superpowers/references/copilot-tools.md
```

Then inspect the installed skills and remove or rewrite every Gemini- or Copilot-related reference. The intent is to avoid shipping any Gemini- or Copilot-specific tooling, instructions, examples, or platform guidance because this project does not use either of them. Choose the appropriate search and edit approach based on the workspace and available tools.
...
Use the section below verbatim.
