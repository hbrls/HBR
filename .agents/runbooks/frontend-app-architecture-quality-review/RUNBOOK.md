...
本技能从前端应用的六个核心构成部分出发，对架构质量进行系统审查：可观测（Observability）、Build（构建与依赖管理）、基础框架（Base Framework）、Utils（工具层）、业务组件/模块、第三方库（集成与隔离）。此外，「依赖库（Dependency Inventory）」作为审查的事实底座先行执行，为各 Phase 提供统一的依赖清单与质量画像。
...
依赖库作为审查的事实底座，先行于各 Phase 执行：定位项目全部依赖声明文件，枚举所有显式/第一层依赖（不展开传递依赖），并逐个分析其功能、作用、潜在优势与潜在劣势，识别版本治理、能力重叠、僵尸依赖、安全合规与供应链等系统性风险，为后续各 Phase 提供统一的依赖清单与质量画像。

详细说明见：`templates/TASK-120-依赖库.md`
...
| Phase | 模板文件 | TASK 文件 |
|-------|---------|---------|
| 依赖库 | `templates/TASK-120-依赖库.md` | `.context/TASK-120-依赖库.md` |
